"""
The retired coach prompt lineages, frozen.

Three families that no longer change: the legacy structured ``coach_report_v1..v10``
(schema 1.2), the ``coach_message_v1..v14`` addendum chain (schema 2.0), and the
``coach_message_lean_v1`` disposition-first rewrite the live grouped lineage grew out
of. They are here because a stored ``CoachReport`` is keyed
``(activity_id, prompt_id, schema_version)`` and a prompt rollback is a pure config
flip, so every one of them must still render byte-for-byte as it did when it was
written -- not because any of them is still being developed.

Nothing in here is composed or edited. ``prompts.py`` registers these strings
alongside the composed live lineage and is where the assembly, the registry and the
derived views live; this module is text.
"""
...
# The registry halves this module contributes. `prompts.PROMPT_VERSIONS` and
# `prompts._OPENER_PROMPTS` merge these with the composed live lineage, so a retired
# id resolves exactly as it always did.
ARCHIVED_PROMPT_VERSIONS = {
    ...
    # #443 stream-view-aware prompt (= v9 + the static STREAM-VIEW addendum).
    ...
    # The lean, disposition-first ground-up rewrite (NOT in the vN chain). Same twelve
    # capabilities as v14 (identical pack), radically shorter prose. The live grouped
    # lineage in prompt_clauses.py grew out of this prose.

...
**Amended 2026-08-19 (#825). The thread turn stays at prompt time, and the number is now on the record.** The latency objection had only ever been asserted; `RewriteOutcome.duration_ms` was added to measure it. Measured over 30 rewrites of real stored baselines through the production path (three recorded hard cases, six characters, two runs), the pass costs **5.7 s to 13.5 s, median 9.5 s**. A report is written by a background job and nobody is waiting for it, so that cost is invisible there. A conversational reply is a person watching a cursor, and the turn already buffers the whole reply before streaming a character of it — so a rewrite does not overlap the wait, it doubles it. Roughly ten seconds added to every reply is not a trade this surface can absorb, and no amount of tuning changes the order of magnitude. The objection holds.

That leaves the risk this ADR exists to remove unmitigated on one surface, and it is named rather than papered over: on the thread turn a *declared* voice still reaches generation, so it could in principle reach the substance. What is done about it is instructional and tested rather than structural. The conversational authority-tiering already briefs voice as delivery-only; #825 extends that briefing to cover **what the coach offers to write**, which it did not, and which matters because the thread turn is the one surface where the coach can mint a proposed action. A cross-voice behavioural test over the real conversational path is the companion evidence, the sibling of the report's own invariance test.

The asymmetry is deliberate and worth stating plainly: on the report the guarantee is structural and needs no trust; in the conversation it is instructed and rests on the floor being restated where the voice arrives. If a voice is ever observed steering conversational substance, the lever is `COACH_VOICE_BLOCK_ENABLED=false`, and the fix is this seam, at whatever latency cost is judged acceptable then.

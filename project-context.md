...
The context pack carries a `longitudinal` section (M4): a token-bounded digest of the runner's last 1-2 non-fallback reports (date, headline, `lead_argument`, recommended `next_steps` only, never the full body) plus the matching `RunnerBaseline` bucket trend for this run's `effort|terrain|temp-band` context (omitted while that bucket still abstains), so the report advances the prior narrative instead of restating it; the active prompt `coach_report_v2` adds the "advance, do not restate" discipline (rule 16) while retaining `coach_report_v1` for prior cached reports.
...
Anthropic access requires `ANTHROPIC_API_KEY`; the coach model id and prompt id are configured via `COACH_MODEL_ID` and `COACH_PROMPT_ID` (default `coach_report_v2`).

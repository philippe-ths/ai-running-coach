...
The active prompt is `coach_report_v5` (default): it adds the "advance, do not restate" discipline (rule 16, M4), the perceived-effort RPE-weighting discipline (rule 17, M6), the adherence "advise, do not nag" discipline (rule 18, M7), and the believed-facts "apply, hedge by tags, never override today's data" discipline (rule 19, M8), while retaining byte-stable `coach_report_v1`..`coach_report_v4` for prior cached reports. Output schema is unchanged across v1-v5 (SCHEMA_VERSION stays 1.2); only `prompt_id` advances, which the M0 versioned cache uses to regenerate and retain history.
...
Anthropic access requires `ANTHROPIC_API_KEY`; the coach model id and prompt id are configured via `COACH_MODEL_ID` and `COACH_PROMPT_ID` (default `coach_report_v5`).
...
`backend/app/services/coach/` owns the LLM coach (`context.py`, `prompts.py`, `llm.py`, `validator.py`, `service.py`, `chat.py`, `perceived_effort.py` the M6 RPE-vs-HR divergence + pain-trend signal, `adherence.py` the M7 next-step-outcome labelling, `beliefs.py` + `belief_store.py` the M8 durable belief write-back/retrieval over the `CoachingContext` table); its `eval/` subpackage is the M5 offline eval harness (`rubric.py` the five deterministic scoring assertions, `harness.py` the DB-load + aggregation + regression-compare, `fixtures.py` synthetic good/bad reports for self-validation).

...
The active prompt is `coach_report_v4` (default): it adds the "advance, do not restate" discipline (rule 16, M4), the perceived-effort RPE-weighting discipline (rule 17, M6), and the adherence "advise, do not nag" discipline (rule 18, M7), while retaining byte-stable `coach_report_v1`/`coach_report_v2`/`coach_report_v3` for prior cached reports. Output schema is unchanged across v1-v4 (SCHEMA_VERSION stays 1.2); only `prompt_id` advances, which the M0 versioned cache uses to regenerate and retain history.
...
Anthropic access requires `ANTHROPIC_API_KEY`; the coach model id and prompt id are configured via `COACH_MODEL_ID` and `COACH_PROMPT_ID` (default `coach_report_v4`).

...
The active prompt is `coach_report_v6` (default): it adds the "advance, do not restate" discipline (rule 16, M4), the perceived-effort RPE-weighting discipline (rule 17, M6), the adherence "advise, do not nag" discipline (rule 18, M7), the believed-facts "apply, hedge by tags, never override today's data" discipline (rule 19, M8), and the calibrated-correction + non-diagnostic-referral discipline (rule 20, M9), while retaining byte-stable `coach_report_v1`..`coach_report_v5` for prior cached reports. Output schema is unchanged across v1-v6 (SCHEMA_VERSION stays 1.2); only `prompt_id` advances, which the M0 versioned cache uses to regenerate and retain history.
...
Anthropic access requires `ANTHROPIC_API_KEY`; the coach model id and prompt id are configured via `COACH_MODEL_ID` and `COACH_PROMPT_ID` (default `coach_report_v6`).
...
The offline eval harness (M5) under `app/services/coach/eval/` is the deterministic quality gate for the long-horizon learning milestones (M7-M10): it scores each `CoachReport` independently from its stored `report` + `context_pack` against five rubric assertions (led-with-headline, discounted-inflated-HR, no-medical-overreach reusing validator rule 5, advanced-not-parroted, abstained-on-thin-trend), aggregates a repeatable scorecard scoped to the current `(prompt_id, schema_version)`, and flags regressions across versions; driven by `backend/scripts/eval_coach_reports.py` (the `make eval` / `make eval-selftest` targets). The procedure is documented in `docs/testing/coach-report-eval.md`.
...
`backend/app/models/` holds one ORM model per file (`activity`, `activity_stream`, `checkin`, `coach_chat_message`, `coach_report`, `coaching_context`, `derived_metric`, `runner_baseline`, `strava_account`, `user`, `user_profile`) with a barrel `__init__.py`.
...
`backend/app/services/coach/` owns the LLM coach (`context.py`, `prompts.py`, `llm.py`, `validator.py`, `service.py`, `chat.py`, `perceived_effort.py` the M6 RPE-vs-HR divergence + pain-trend signal, `adherence.py` the M7 next-step-outcome labelling, `beliefs.py` + `belief_store.py` the M8 durable belief write-back/retrieval over the `CoachingContext` table, `calibration.py` the M9 self-calibrating HR-drift correction + non-diagnostic referral); its `eval/` subpackage is the M5 offline eval harness (`rubric.py` the five deterministic scoring assertions, `harness.py` the DB-load + aggregation + regression-compare, `fixtures.py` synthetic good/bad reports for self-validation).
